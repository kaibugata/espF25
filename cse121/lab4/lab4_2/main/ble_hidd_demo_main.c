...
void hid_demo_task(void *pvParameters)
{
    ...
    while(1) {
	   ...
	   for(int i = 0; i < 50; i ++){
	      esp_hidd_send_mouse_value(hid_conn_id,HID_MOUSE_LEFT,-1,0);
	   }
              vTaskDelay(2000 / portTICK_PERIOD_MS);
            if (send_volum_up) {
		send_volum_up = false;
            ...
            for(int j = 0; j < 50; j++){   
	       	esp_hidd_send_mouse_value(hid_conn_id, HID_MOUSE_RIGHT,1,0);
	    }   
		//esp_hidd_send_consumer_value(hid_conn_id, HID_CONSUMER_VOLUME_DOWN, true);
                vTaskDelay(2000 / portTICK_PERIOD_MS);
               // esp_hidd_send_mouse_value(hid_conn_id, HID_MOUSE_RIGHT,0,0);
