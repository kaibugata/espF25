...
void app_main(void)
{
    ...
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(10));//initial val 1000
	...
	if(lowCounter != 0){//if led has been off for some time
		if(lowCounter >= 18  && lowCounter < 21 && noiseCounter == 0){//test for /
			//ESP_LOGI(LED, "/");
			processSymbol('/');
			//printf("%s\n",morseResult);
			if(morseResult[0] != '\0'){
    	...
    	noiseCounter = 0;
	if(highCounter >= 3 && highCounter <= 6){//.
		//ESP_LOGI(LED, ".");
		processSymbol('.');
		highCounter = 0;
	}else if(highCounter > 13  && highCounter <= 16){//-
		//ESP_LOGI(LED, "-");
		processSymbol('-');
		highCounter = 0;
	}
	...
	if(lowCounter == 25){// "  " 25 is experimental value could be higher since / is 20
		//ESP_LOGI(LED, "____");
		...
		highCounter = 0;
	}else if(lowCounter == 45){//end of entire text  try 45 as test val, 40 worked half the time
		//ESP_LOGI(LED, "!");
    ...
    //14 or 15   = dash(if adc >= 21)
    //4 or 5  = dot(if adc >= 21)
    //approx 19 or 20   = space between letters (if adc < 21)
    //approx    = space between words (if adc < 21)
}
