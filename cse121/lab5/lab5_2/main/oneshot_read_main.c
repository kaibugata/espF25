...
void app_main(void)
{
    ...
    //
    //
    //CURRENT RATE: APPROX 1 char/2.11 sec
    //
    //
    //
    ...
    while (1) {
        ...
        vTaskDelay(pdMS_TO_TICKS(75));//initial val 1000
	...
	if(lowCounter != 0){//if led has been off for some time
		if(lowCounter >= 10  && lowCounter < 15 && noiseCounter == 0){//test for /
			//ESP_LOGI(LED, "/");
			processSymbol('/');
			//printf("%s\n",morseResult);
			if(morseResult[0] != '\0'){
    	...
    	noiseCounter = 0;
	if(highCounter >= 2 && highCounter <= 5){//.
	//	ESP_LOGI(LED, ".");
		processSymbol('.');
		highCounter = 0;
	}else if(highCounter > 5  && highCounter <= 10){//-
	//	ESP_LOGI(LED, "-");
		processSymbol('-');
		highCounter = 0;
	}
	...
	if(lowCounter == 20){// "  "
		...
		highCounter = 0;
	}else if(lowCounter == 40){//end of entire text
    ...
    //approx 25 = space between words (if adc < 21)
}
